...
import { EmlakOdeme, Emlak, Sozlesme, Varlik, GiderTuru, Ayarlar } from '../types';

interface EmlakOdemeMakbuzSablonuProps {
  odeme: EmlakOdeme;
  emlak: Emlak;
  sozlesme: Sozlesme;
  varlik: Varlik;
...
const EmlakOdemeMakbuzSablonu: React.FC<EmlakOdemeMakbuzSablonuProps> = ({ odeme, emlak, sozlesme, varlik, giderTuru, ayarlar }) => {
  const { sirketBilgileri, logo } = ayarlar;
  const varlikAdi = varlik.type === 'musteri' ? `${varlik.isim} ${varlik.soyisim}` : varlik.sirketAdi;
  const sembol = ayarlar.paraBirimleri.find(p => p.kod === odeme.paraBirimi)?.sembol || odeme.paraBirimi;
  const emlakAdi = `${emlak.blok} Blok, Kat ${emlak.kat}, No ${emlak.daireNo}`;
    ...
    <div className="w-[800px] bg-white p-12 font-sans text-gray-800">
      <header className="flex justify-between items-start pb-6 border-b-2 border-green-600">
        ...
        <div className="text-right">
          ...
          <p className="mt-2"><span className="font-semibold">Makbuz No:</span> ODEME-{odeme.id}</p>
          <p><span className="font-semibold">Ödeme Tarihi:</span> {odeme.odemeTarihi ? new Date(odeme.odemeTarihi).toLocaleDateString('tr-TR') : '-'}</p>
        </div>
      </header>
            ...
            <tr>
              <td className="p-3 border-b">
                <p className="font-semibold text-slate-900">
                  {giderTuru.ad} - {emlakAdi}
                </p>
                <p className="text-sm text-gray-500">Sözleşme: {sozlesme.baslik}</p>
              </td>
...
export default EmlakOdemeMakbuzSablonu;